...
export const getContact = createAsyncThunk(
  ...
  async (id, { dispatch, getState }) => {
    try {
      const response = await axios.get(`${url}/users/Parking/${id}`);
...
if (!contact.admin || Object.keys(contact.admin).length === 0 || contact.admin.passwordAdmin === "") {
    ...
    response = await axios.post(`${url}/users/Parking/create`, contact);
} else {
    // Otherwise, use the /Auth/create endpoint
    response = await axios.post(`${url}/Auth/create`, contact);
}
    const id = await response.data;
    const response1 = await axios.get(`${url}/users/Parking/${id}`);
    ...
    console.log('record = ');
    console.log(record);
    const response2 = await axios.post(`${url}/records/Parking/create_record`,record );
...
export const updateContact = createAsyncThunk(
  ...
  async (contact, { dispatch, getState }) => {
    ...
    // Retrieve the adminID from the user data
    const adminID = user.id;
    const response = await axios.put(`${url}/users/Parking/${contact.id}`, contact);
    console.log(response)
    const response1 = await axios.get(`${url}/users/Parking/${contact.id}`);
    const data = await response1.data;
...
export const removeContact = createAsyncThunk(
  ...
  async (id, { dispatch, getState }) => {
    ...
    const response = await axios.delete(`${url}/users/Parking/${id}`);
